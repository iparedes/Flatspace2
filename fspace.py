...
class Body:
    ...
    def set_pos_time(self, t):
        ...
        x = self.orbit.pos.x + newx
        y = self.orbit.pos.y + newy
        pos = Pos(x, y)
        self.pos = pos
        return pos
...
class Planet(Body):
    def __init__(self, primary=None, name="", mass=0, radius=0,peri=0,apo=0,incl=0,init_pos=0):
        ...
        self.pos=self.pos_at_angle(init_pos)
...
class Orbit():
    ...
    def __str__(self):
        t="a:"+str(self.a)+", "+"b:"+str(self.b)+"\n"
        r=self.area()
        t+="top:"+str(r.top)+", left:"+str(r.left)+", bottom:"+str(r.bottom)+", right:"+str(r.right)+"\n"
        return t
